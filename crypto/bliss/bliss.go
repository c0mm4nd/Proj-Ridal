...
type blissDSA struct {
	// Private keys
	newPrivateKey    func(s1, s2, a *poly.PolyArray) Crypto.PrivateKey
	privKeyFromBytes func(pk []byte) (Crypto.PrivateKey, Crypto.PublicKey)
	privKeyBytesLen  func() int

	// Public keys
	newPublicKey   func(a *poly.PolyArray) Crypto.PublicKey
	parsePubKey    func(pubKeyStr []byte) (Crypto.PublicKey, error)
	pubKeyBytesLen func() int

	// Signatures
	newSignature      func(z1, z2 *poly.PolyArray, c []uint32) Crypto.Signature
	parseDERSignature func(sigStr []byte) (Crypto.Signature, error)
	parseSignature    func(sigStr []byte) (Crypto.Signature, error)
	recoverCompact    func(signature, hash []byte) (Crypto.PublicKey, bool, error)

	//
	generateKey func(rand io.Reader) (Crypto.PrivateKey, Crypto.PublicKey, error)
	sign        func(priv Crypto.PrivateKey, hash []byte) (Crypto.Signature, error)
	verify      func(pub Crypto.PublicKey, hash []byte, sig Crypto.Signature) bool
...
// Private keys
func (sp blissDSA) NewPrivateKey(s1, s2, a *poly.PolyArray) Crypto.PrivateKey {
	return sp.newPrivateKey(s1, s2, a)
}

func (sp blissDSA) PrivKeyFromBytes(pk []byte) (Crypto.PrivateKey, Crypto.PublicKey) {
...
// Public keys
func (sp blissDSA) NewPublicKey(a *poly.PolyArray) Crypto.PublicKey {
	return sp.newPublicKey(a)
}

func (sp blissDSA) ParsePubKey(pubKeyStr []byte) (Crypto.PublicKey, error) {
...
// Signatures
func (sp blissDSA) NewSignature(z1, z2 *poly.PolyArray, c []uint32) Crypto.Signature {
	return sp.newSignature(z1, z2, c)
}

func (sp blissDSA) ParseDERSignature(sigStr []byte) (Crypto.Signature, error) {
	return sp.parseDERSignature(sigStr)
}

func (sp blissDSA) ParseSignature(sigStr []byte) (Crypto.Signature, error) {
	return sp.parseSignature(sigStr)
}

func (sp blissDSA) RecoverCompact(signature, hash []byte) (Crypto.PublicKey, bool,
	error) {
	return sp.recoverCompact(signature, hash)
}

// ECDSA
func (sp blissDSA) GenerateKey(rand io.Reader) (Crypto.PrivateKey, Crypto.PublicKey,
	error) {
	return sp.generateKey(rand)
}

func (sp blissDSA) Sign(priv Crypto.PrivateKey, hash []byte) (Crypto.Signature, error) {
	return sp.sign(priv, hash)
}

func (sp blissDSA) Verify(pub Crypto.PublicKey, hash []byte, sig Crypto.Signature) bool {
